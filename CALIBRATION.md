...
- **Uma linha de log por request.** É o que o `pino-http` faz, e o simulador desenha o fluxo de log com a **mesma compressão** que já usa para as requests (`packetsPerSecond`). Então um pacote de log corresponde ao mesmo punhado de requests que um pacote de request — a proporção entre os dois fluxos é real, e uma task sem tráfego não envia log de aplicação nenhum.
