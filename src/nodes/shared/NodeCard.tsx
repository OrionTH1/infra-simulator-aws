...
export function NodeCard({ variant, icon, title, tooltip, status = 'idle', children }: NodeCardProps) {
  const borderClass =
    status !== 'idle' ? statusBorderClass[status] : variant === 'interaction' ? 'border-border-interaction' : 'border-border'

  return (
    <div className={`min-w-[170px] overflow-visible rounded-card border bg-surface shadow-card ${borderClass}`}>
